...
export class SessionSubagentHost {
  ...
  private resolveSpawnBinding(
    ...
    if (
      ...
      profile.model !== parent.config.modelAlias
    ) {
      const modelAlias = profile.model;
      const providerManager = this.session.options.providerManager;
      try {
        providerManager?.resolveProviderConfig(modelAlias);
      } catch (error) {
        throw wrapSubagentModelError(error, modelAlias, parent.config.modelAlias);
      }
      return {
        modelAlias,
