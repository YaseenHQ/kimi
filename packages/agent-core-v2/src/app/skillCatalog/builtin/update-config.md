---
name: update-config
description: Inspect or edit Echadron's own config — `config.toml` (model, provider, permission, hooks) and `tui.toml` (theme, editor, notifications, auto-update). Use when the user asks what a setting does or wants to change one.
---

# Configure Echadron (update-config)

Help the user inspect, change, and validate Echadron's configuration files. The files are **TOML** with **snake_case** keys.

## The two config files

Echadron has two TOML config files, both under `<ECHADRON_HOME>/`, both snake_case, but with different ownership — decide which one the user means before doing anything.

The runtime resolves the data directory as `ECHADRON_HOME` first, then the internal `ECHADRON_CODE_HOME` alias, then legacy `IMPERIUM_HOME` or `KIMI_CODE_HOME`, falling back to `~/.echadron`. Before doing anything, resolve the actual directory with Bash so you don't write to the wrong place. Check the modern variable first, then the aliases:

```bash
echo "$ECHADRON_HOME"
echo "$ECHADRON_CODE_HOME"
echo "$IMPERIUM_HOME"
echo "$KIMI_CODE_HOME"
echo "$HOME/.echadron"
```
...
- Location: `<ECHADRON_HOME>/config.toml` or `<ECHADRON_HOME>/tui.toml`. For other scopes/files, defer to the official docs.
...
3. **Copy out a candidate (do not create from scratch)**: use **Bash** to copy the target verbatim — `cp config.toml config-new.toml` (same directory, `-new` suffix; for tui.toml, `cp tui.toml tui-new.toml`). **Leave the original untouched for now.**
   - Only when the target doesn't exist (nothing to copy) should you use **Write** to create a minimal skeleton candidate (e.g. just the comment line `# <ECHADRON_HOME>/config.toml`).
...
5. **Validate the candidate** (see Capability 3, via `echadron doctor`). **If anything fails, keep Editing the candidate and re-validate, looping until it all passes.**
...
Use **`echadron doctor`** to validate the candidate you wrote — it doesn't start the TUI and doesn't modify any file; it runs Echadron's own parser + schema (syntax and schema together), so it's the authoritative check. Pick the subcommand by which file you changed, and pass the **candidate** path explicitly:

- changed `config.toml` → `echadron doctor config <config-new.toml path>`
- changed `tui.toml` → `echadron doctor tui <tui-new.toml path>`
...
Then do two checks `echadron doctor` can't:
...
> To also check whether the currently **active** config is OK overall, run `echadron doctor` with no path (it checks the default `config.toml` + `tui.toml`, showing a missing one as skipped).
