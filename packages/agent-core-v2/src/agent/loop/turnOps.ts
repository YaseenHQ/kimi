 ...
 * Also persists the terminal `turn.ended` record (reason / error / durationMs)
 * so downstream history rebuilds can recover how a turn ended; the record
 * carries no engine-restorable state, so its `apply` is a no-op. The
