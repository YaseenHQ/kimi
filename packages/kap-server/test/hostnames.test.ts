...
describe('formatHostErrorMessage', () => {
  it('includes the rejected host and allow guidance', () => {
    expect(formatHostErrorMessage('APP.Example.com:443')).toBe(
      "Invalid Host header: app.example.com; allow this host with ECHADRON_ALLOWED_HOSTS=app.example.com or 'echadron web --allowed-host app.example.com'.",
    );
  });
});
...
describe('createHostCheck (onRequest hook)', () => {
  ...
  it('rejects a disallowed Host with the 40301 envelope', async () => {
    ...
    expect(body['msg']).toBe(
      "Invalid Host header: evil.com; allow this host with ECHADRON_ALLOWED_HOSTS=evil.com or 'echadron web --allowed-host evil.com'.",
    );
