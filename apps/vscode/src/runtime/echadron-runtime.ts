...
export interface EchadronRuntimeOptions {
...
export class EchadronRuntime {
  readonly harness: KimiHarness;

  private readonly broadcast: RuntimeBroadcast;
  private readonly captureBaseline: EchadronRuntimeOptions["captureBaseline"];
  private readonly log: EchadronRuntimeOptions["log"];
  private readonly sessions = new Map<string, SessionRuntime>();
  private readonly sessionByView = new Map<string, string>();
  private closed = false;

  constructor(options: EchadronRuntimeOptions) {
  ...
  private ensureOpen(): void {
    if (this.closed) throw new Error("Echadron runtime is closed.");
  }
}
