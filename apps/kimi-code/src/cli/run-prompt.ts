...
export async function runPrompt(
  ...
  const cleanupPromptRun = async (): Promise<void> => {
    ...
    // the background if it overruns; the caller (`echadron -p`) force-exits shortly
  ...
  try {
    ...
    // Headless goal mode: `echadron -p "/goal <objective>"`. The goal driver keeps
      ...
      stderr.write(
        `${chalk.hex('#E8A838')(
          `Session "${opts.session}" was created under a different directory.\n` +
            `  cd "${target.workDir}" && echadron -r ${opts.session}`,
        )}\n\n`,
      );
      throw new Error(
        `Session "${opts.session}" was created under a different directory.`,
...
function runPromptTurn(
  ...
  // A `echadron -p` run is not done just because the model ended a turn: an active
