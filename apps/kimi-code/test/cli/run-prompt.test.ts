        ...
        if (outputFormat === 'stream-json') {
          ...
          stdout.write(
            `${JSON.stringify({
              ...
              command: 'echadron -r ses_prompt',
              content: 'To resume this session: echadron -r ses_prompt',
            })}\n`,
          );
          return;
        }
        stderr.write(`echadron version ${version}\n`);
        stdout.write('• hello world\n\n');
        stderr.write('To resume this session: echadron -r ses_prompt\n');
      },
    ),
...
describe('runPrompt', () => {
  ...
  it('creates a fresh auto-permission session and streams assistant output to stdout', async () => {
    ...
    expect(stderr.text()).toBe('To resume this session: echadron -r ses_prompt\n');
  ...
  it('formats thinking and assistant output as transcript blocks', async () => {
    ...
    expect(stderr.text()).toBe(
      '• The user wants an exact reply.\n  No tools are needed.\n\nTo resume this session: echadron -r ses_prompt\n',
    );
  ...
  it('writes stream-json output as assistant JSONL with resume meta without transcript bullets', async () => {
    ...
    expect(stdout.text()).toBe(
      [
        ...
        '{"role":"meta","type":"session.resume_hint","session_id":"ses_prompt","command":"echadron -r ses_prompt","content":"To resume this session: echadron -r ses_prompt"}',
        '',
      ].join('\n'),
    );
    expect(stderr.text()).toBe('');
  ...
  it('throws when no default model is configured', async () => {
    ...
    await expect(
      ...
      'No model configured. Run `echadron` and use /login to sign in, then retry; or set default_model in config.toml.',
    );
  ...
  it('emits the version first in text mode when the experimental flag is enabled', async () => {
    ...
    expect(stderr.text().startsWith('echadron version 1.2.3-test\n')).toBe(true);
    expect(stdout.text()).toBe('• hello world\n\n');
  });
  ...
  it('does not emit the version when the experimental flag is disabled', async () => {
    ...
    expect(stderr.text()).not.toContain('echadron version');
  });
