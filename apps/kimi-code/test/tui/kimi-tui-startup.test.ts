...
vi.mock('#/tui/commands/prompts', async (importOriginal) => {
  ...
  return {
    ...
    promptProviderConfigurationRemoval: vi.fn(),
    promptConfirmProviderConfigurationRemoval: vi.fn(async () => false),
  };
});
...
describe('KimiTUI startup', () => {
  ...
  it('does not resume a session from a different cwd and shows a cd hint', async () => {
    ...
    expect(driver.state.activeDialog).toBeNull();
    const expectedResumeCmd = `cd ${quoteShellArg('/tmp/proj-b')} && echadron --resume ${quoteShellArg('ses-other-cwd')}`;
  ...
  it('copies a shell-safe resume command for another cwd with metacharacters', async () => {
    ...
    expect(resumeSession).not.toHaveBeenCalled();
    const expectedResumeCmd = `cd ${quoteShellArg('/tmp/proj$(touch /tmp/pwned)')} && echadron --resume ${quoteShellArg('ses-other-cwd')}`;
    expect(copyTextToClipboardMock).toHaveBeenCalledWith(expectedResumeCmd);
    const transcript = driver.state.transcriptContainer.render(160).join('\n');
    expect(transcript).toContain(`To resume, run: ${expectedResumeCmd}`);
  });
      ...
      auth: {
        status: vi.fn(async (providerName: string) => ({
          providers: [
            {
              providerName,
              hasToken: providerName === 'managed:kimi-code' || providerName === 'xai',
            },
          ],
        })),
        login: vi.fn(async () => {}),
  ...
  it('offers stale managed configuration for explicit removal', async () => {
    const session = makeSession();
    const removeProvider = vi.fn(async () => {});
    const harness = makeHarness(session, {
      getConfig: vi.fn(async () => ({
    ...
    vi.mocked(promptLogoutProviderSelection).mockResolvedValue('__remove_provider_configuration__');
    vi.mocked(promptProviderConfigurationRemoval).mockResolvedValue('provider:managed:kimi-code');
    vi.mocked(promptConfirmProviderConfigurationRemoval).mockResolvedValueOnce(true);
    await handleLogoutCommand(driver as any);
