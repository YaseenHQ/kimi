...
| `model` | 否 | 此 Profile 作为子 Agent 启动时使用的已配置模型别名。省略或写 `inherit` 时继承调用方当前模型 |
...
子 Agent 启动时会固定模型绑定，恢复同一个子 Agent 时也会重新应用该绑定。它不会修改父 Agent 的模型、系统提示词或工具列表，因此把任务委派给其他模型不会破坏父 Agent 的提示词缓存前缀。
