...
OAuth token 与 `config.toml` 分开存储；供应商引用和模型元数据仍使用 Kimi 现有配置格式。`/logout` 会逐一列出所有已连接供应商，也可清除 **All OAuth accounts**、**All API-key providers** 或 **All credentials**。每个组合都会显示所包含的供应商。普通登出只清除凭据，并保留供应商、模型配置和已保存会话。活动 OAuth 会话保持打开；活动 API key 会话会关闭，以免在内存中继续保留已清除的 key。
